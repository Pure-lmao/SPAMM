//! Admin-only write of arbitrary bytes to a program-owned account (dev tooling).
//!
...
use spamm_aggregator::helpers::{get_rent_local, verify_signer, verify_system_program};
...
pub fn process(_program_id: &Address, accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   let [admin, config_pda, data_pda, system_program] = accounts else {
