...
//!
//! **Account data** (oracle account): `[discriminator u8][bump u8][sequence u32][payload T]`
//! starting at `ACCOUNT_DATA_BASE` (`0x28c0`), matching the SPAMM oracle account layout.
//!
//! **Instruction data**: `[discriminator u8][sequence u32][payload T]` starting at `IX_DATA_BASE`
...
/// After account `discriminator` + `bump`: `sequence: u32`.
const ORACLE_SEQUENCE: usize = ACCOUNT_DATA_BASE + 2;

/// Immediately after `sequence: u32`: `payload: T`.
const ORACLE_PAYLOAD: usize = ORACLE_SEQUENCE + 4;
...
impl<T: Sized + Copy> Oracle<T> {
   /// After ix discriminator: `sequence: u32`.
   const IX_SEQUENCE: usize = IX_DATA_BASE + 1;

   /// After `sequence: u32`: `payload: T`.
   const IX_PAYLOAD: usize = IX_DATA_BASE + 1 + 4;
