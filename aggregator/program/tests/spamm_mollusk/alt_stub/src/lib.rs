...
//! Implements `CreateLookupTable` (`0`) and `ExtendLookupTable` (`2`) with the same bincode layout as
//! the real ALT program. There is no on-chain remove-address instruction; aggregator `deregister_mm`
//! does not shrink the ALT.
