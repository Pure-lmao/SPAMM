...
use crate::state::BET_ACCOUNT_LEN;
use crate::state::account_bet::{BET_RESULT_OFFSET};

pub const GRADE_BETS_IX_DISCRIMINATOR: u8 = 4;
...
pub fn process(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   ...
   for i in 0..data_len {
      if bet_accounts[i].data_len() != BET_ACCOUNT_LEN as usize {
         log!("grade_bets: bet account data length mismatch");
         return Err(ProgramError::InvalidInstructionData);
      }
