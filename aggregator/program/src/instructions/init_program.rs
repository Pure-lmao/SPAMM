...
//!
//! Instruction data (after router discriminator): empty.
...
use crate::{
   ID,
   constants::{
      CONFIG_PDA_BUMP, CONFIG_PDA_SEED, MM_LIST_BUMP, MM_LIST_PDA, MM_LIST_PDA_SEED
   },
   helpers::{
      get_rent, verify_config_pda, verify_rent_sysvar, verify_signer, verify_system_program
   },
   state::{
      CONFIG_PDA_DISCRIMINATOR, CONFIG_PDA_LEN, ConfigPdaData,
...
pub fn process(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   ...
   if unlikely(!data.is_empty()) {
      log!("init_program: instruction data must be empty");
   ...
   CreateAccount {
      from: authority,
      to: mm_list_pda,
      lamports: get_rent(rent_sysvar, mm_list_space)?,
      space: mm_list_space,
      owner: &ID,
   }
