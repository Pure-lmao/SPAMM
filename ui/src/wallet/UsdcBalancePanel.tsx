...
import { useAppTransactionSigner } from "./useAppTransactionSigner";
import { signErrorMessageForUi } from "./walletStandardV1Signer";
...
import { subscribeWalletBalanceRefresh } from "./walletBalanceRefresh";
...
export function UsdcBalancePanel(): ReactElement | null {
   ...
   useEffect(() => {
      ...
      void refresh();
      const unsubscribe = subscribeWalletBalanceRefresh(() => {
         void refresh();
      });
      return () => {
   ...
   const runUsdcAirdrop = useCallback(async () => {
      ...
      try {
         ...
         setUsdcAirdropErr(signErrorMessageForUi(e, { walletName }));
      } finally {
         setUsdcAirdropBusy(false);
      }
   }, [account, isConnected, refresh, rpc, rpcSubscriptions, signer, signerReady, walletName]);
