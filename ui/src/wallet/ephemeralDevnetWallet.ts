...
const TX_VERSIONS = ["legacy", 0, 1] as const;
...
/** Kit partial signer — signs v1 txs locally, skipping Connector's 1232-byte v0 check. */
export async function createEphemeralKitSigner(): Promise<TransactionSigner> {
...
export function registerSpammEphemeralDevnetWallet(): () => void {
   ...
   // @ts-expect-error Wallet Standard register is not fully typed for custom wallets.
