//! Create the event-state PDA: `["event_state", event_id]`. Initial `sequence = 0` and zeroed
//! `game_state`; call `update_event_state` before quoting (sequence must be `> 0` on `get_quote`).
//!
...
use spamm_aggregator::state::{
   EVENT_STATE_DISCRIMINATOR, EVENT_STATE_HEADER_LEN, EVENT_STATE_SEED, EventGameState, EventStateDataZc
};
