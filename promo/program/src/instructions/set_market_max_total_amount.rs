//! Admin patch for per-market `max_total_amount` (pool cap across all bets).
//!
//! Accounts **(4)**:
//! 0. `admin` (signer)
...
//!
//! Instruction `data`: `market_id` wire (26) + `max_total_amount` u64 LE.
...
use crate::state::account_oracle::{
   MAX_TOTAL_AMOUNT_OFFSET, ORACLE_DISCRIMINATOR, ORACLE_SIZE, read_discriminator,
};

pub const SET_MARKET_MAX_TOTAL_AMOUNT_IX_DISCRIMINATOR: u8 = 15;
...
pub fn process(program_id: &Address, accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   let [admin, config_pda, mm_market_data_pda, _system] = accounts else {
      log!("set_market_max_total_amount: accounts mismatch");
   ...
   if unlikely(data.len() != PAYLOAD_LEN) {
      log!("set_market_max_total_amount: payload length mismatch");
      return Err(ProgramError::InvalidInstructionData);
   }

   let market_id = MarketId::decode(&data[..MarketId::WIRE_SIZE])
      .ok_or(ProgramError::InvalidInstructionData)?;
   let max_total_amount = unsafe { read_u64_le_unchecked(data.as_ptr(), MarketId::WIRE_SIZE) };
   ...
   if unlikely(mm_market_data_pda.data_len() != ORACLE_SIZE as usize) {
      log!("set_market_max_total_amount: market data wrong length");
   ...
   unsafe {
      ...
      core::ptr::write_unaligned(
         mm_market_data_pda.data_mut_ptr().add(MAX_TOTAL_AMOUNT_OFFSET) as *mut u64,
         max_total_amount,
      );
   }
