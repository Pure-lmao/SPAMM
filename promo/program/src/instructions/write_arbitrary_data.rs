...
//!
//! Grows the account when `offset + payload` exceeds current size (funds rent from admin).
//!
//! Accounts **(5)**:
//! 0. `admin` (writable signer) — must match config admin; pays rent on grow
//! 1. `config_pda` (readonly) — MM `["config"]` for auth
//! 2. `data_pda` (writable) — target (may be the config itself)
//! 3. `rent_sysvar` (readonly)
//! 4. `system_program` (readonly)
//!
//! Instruction `data` after disc 254: `offset` u16 LE + bytes.
...
pub fn process(program_id: &Address, accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   ...
   if unlikely(data.len() < 2) {
      log!("write_arbitrary_data: missing offset");
      return Err(ProgramError::InvalidInstructionData);
   }

   let data_offset = u16::from_le_bytes([data[0], data[1]]) as usize;
   let data_to_write = &data[2..];
   if unlikely(data_to_write.is_empty()) {
      log!("write_arbitrary_data: empty payload");
      return Err(ProgramError::InvalidInstructionData);
   }

   let need_len = data_offset.saturating_add(data_to_write.len());
   let cur_len = data_pda.data_len();
   if need_len > cur_len {
      let new_rent = get_rent(rent_sysvar, need_len as u64)?;
      ...
      data_pda.resize(need_len)?;
   }

   if unlikely(data_pda.data_len() < need_len) {
      return Err(ProgramError::InvalidAccountData);
   }

   unsafe {
      write_arbitrary_bytes_unchecked(data_pda.data_mut_ptr(), data_offset, data_to_write);
   }
