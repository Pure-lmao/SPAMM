...
use spamm_aggregator::state::{MARKET_ID_LEN, MarketId};
...
pub fn process(program_id: &Address, accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
   ...
   verify_mm_config_auth(program_id, auth, config_pda)?;

   if unlikely(data.len() != MarketId::WIRE_SIZE) {
      log!("close_market: instruction data wrong length");
      return Err(ProgramError::InvalidInstructionData);
   }

   let market_id_bytes: &[u8; MARKET_ID_LEN] = data.try_into().unwrap();
   ...
   let (pda, _bump) = find_market_data_pda_from_wire(program_id, market_id_bytes);
