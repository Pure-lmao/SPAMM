...
use spamm_aggregator::state::other::EVENT_GAME_STATE_LEN;
...
/// - `sequence` (`u16`, LE)
/// - `game_state` ([`EventGameState`] wire, 8 bytes)
pub const UPDATE_EVENT_STATE_IX_DATA_LEN: usize = EventId::WIRE_SIZE + 2 + EVENT_GAME_STATE_LEN;
...
impl UpdateEventStateIxPayload {
   ...
   pub fn decode(data: &[u8]) -> Result<Self, ProgramError> {
      ...
      let game_state = EventGameState::decode(&data[EventId::WIRE_SIZE + 2..EventId::WIRE_SIZE + 2 + 8])?;
      Ok(Self {
         event_id,
         sequence,
         game_state,
